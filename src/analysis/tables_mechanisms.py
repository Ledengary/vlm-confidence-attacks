...
def build_within_answer():
    ...
    lines.append(
        ...
        + r") is the twelve-cell mean and includes the constraint-blocked Molmo/POPE cell (adversarial within-answer "
...
def build_long_answer_full():
    ...
    for mi, mo in enumerate(models):
        for ri, ro in enumerate(mo["readouts"]):
            if ri == 0:
                ...
                tail = r" \\"
            lines.append(
                f"{head} & {ro['label']} & {fnum(ro['clean'], 3)} & {fnum(ro['attacked'], 3)} & {tail}"
            )
        if mi != len(models) - 1:
            lines.append(r"\midrule")
    lines += [r"\bottomrule", r"\end{tabular}", ""]
    ...
    lines.append(
        ...
        + r" (LLaVA SAPLMA). Feasible-step rate is the median fraction of decode steps that keep the answer byte-identical ($\approx$3\%); rejects/item-dir is the mean number of fresh-decode rejections per item-direction (8 to 16).}"
    )
